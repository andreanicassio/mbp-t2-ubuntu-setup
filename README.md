...
| AirDrop / AWDL (**experimental, opt-in**, not in `install.sh`) | OWL/OpenDrop need monitor mode + injection, which brcmfmac cannot do; but Apple's Broadcom firmware has the whole AWDL engine built in, driven by private iovars | `awdl/`: brcmfmac patch that turns the firmware's AWDL bsscfg into an `awdl0` netdev and forwards AWDL events to userspace, `brcmiovar.py` (raw iovars over nl80211), `awdl-up.sh`/`awdl-down.sh`. Firmware runs AWDL, syncs to a nearby iPad and receives its frames (`awdlparse.py` decodes hostname, version, services); our own service announcement, peer data path and OpenDrop still to do — see `awdl/NOTES.md` |
