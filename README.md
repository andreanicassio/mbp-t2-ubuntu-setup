...
| Speech-to-text (Handy) on Wayland | needs a virtual keyboard and a system-level shortcut | ydotool 1.0 built from source (+ service); Handy AppImage autostarted hidden; **tap Right Option alone** to start/stop (`handy-ralt-toggle` user service reads evdev and sends Handy SIGUSR2; held with another key it stays Option/AltGr), Ctrl+Super+H as a fallback; toggle mode, paste via clipboard + Shift+Insert (`ydotool type` drops accented letters) |
...
- Handy logs `sample count: 0` for any recording with no speech: the count is taken after voice-activity detection, so it does not mean the mic is dead.
- Don't bind Handy to `Handy.AppImage --toggle-transcription`: each press mounts a new 130 MB AppImage and it can hang; `pkill -USR2 -n -x handy` is instant.
